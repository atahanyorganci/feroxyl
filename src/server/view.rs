...
markup::define! {
    ...
    SearchResultFragment<'a>(results: &'a [RankedSearchResult], elapsed: f64, indices: usize) {
        @let elapsed_str = format!("{} entries retrieved in {:.2}s across", results.len(), elapsed);
        div[
    ...
    SearchShell<'a>(query: &'a str) {
        html {
            ...
            body["class" = "min-h-screen bg-stone-50 flex flex-col"] {
                div[
                ...
                div["class" = "relative flex flex-col min-h-screen bg-stone-50"] {
                    ...
                    footer["class" = "py-6 border-t border-stone-200 text-center"] {
                        p["class" = "text-xs font-sans text-slate-500 tracking-wide"] {
                            "No tracking · No profiling · Private by design"
                        }
                    }
                }
            }
...
async fn search(Query(SearchQuery { query, .. }): Query<SearchQuery>) -> impl IntoResponse {
    let body = Body::from_stream(async_stream::stream! {
        ...
        let fragment = SearchResultFragment { results: &results, elapsed: start.elapsed().as_secs_f64(), indices: DEFAULT_PROVIDERS.len() };
        let script = format!(
            r#"<script>
